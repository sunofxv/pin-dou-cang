...
(async () => {
  ...
  // ---- Phase D: 导出列预设（勾选 + 调整顺序 + 复制输出受预设控制）----
  {
    const seed = baseSeed();
    seed.restockRecords = [{ id: 'rs_d1', name: '我的清单', status: 'pending', createdAt: Date.now(), updatedAt: Date.now(), stockedAt: null,
      items: [
    ...
    // 复制清单（按预设：应不含记录名列，且顺序为 色号/份数/颗数/每份颗数）
    await page.click('#rs-copy'); await sleep(400);
    const clip = await page.evaluate(() => navigator.clipboard.readText().catch(() => '')).catch(() => '');
    console.log('EXPORT_CFG:', JSON.stringify({ hasCfg: !!hasCfg, cfgSaved, clip: clip.split('\n') }));
    try { await browser.close(); } catch (e) {}
    const expected = ['colorNumber', 'beads', 'portions', 'perQty']; // record 取消，beads 上移两次
    const cfgOK = errors.length === 0 && !!hasCfg &&
      JSON.stringify(cfgSaved) === JSON.stringify(expected);
    // 复制输出：表头应不含「记录名」，且列顺序与预设一致
    ...
    const exportOK = cfgOK && clipOK;
    console.log(exportOK ? '✅ EXPORT-CFG PASS' : '❌ EXPORT-CFG FAIL');
    if (!exportOK) { console.log('PAGEERRORS:', errors); process.exitCode = 1; }
  }
})();
