...
# 产物：PinDouDouCang.app（双击打开系统默认浏览器访问网站）+ PinDouDouCang-mac.zip（可分发包）
set -e

APP_NAME="PinDouDouCang"
DISPLAY_NAME="拼豆豆仓"
...
cat > "$DIR/MacOS/$APP_NAME" <<'EOF'
#!/usr/bin/env bash
open "__URL__"
EOF
# 用真实地址替换占位符
sed -i '' "s|__URL__|$URL|g" "$DIR/MacOS/$APP_NAME"
chmod +x "$DIR/MacOS/$APP_NAME"
